...
import { ActivatedRoute, Router } from '@angular/router'; // Inyectamos Router
...
onEstadoChanged(nuevoEstado: string) {
  ...
  if (nuevoEstado === 'PROGRAMADA') {
    ...
    this.citasService.reagendarCita(id, fechaHoraOriginal).subscribe({
      next: () => {
        ...
        Swal.fire('¡Éxito!', 'La cita ha sido restaurada a estado PROGRAMADA con éxito.', 'success');
      },
      error: (err) => {
  ...
  // Flujo normal para pasar a FINALIZADA o CANCELADA
  ...
  redirigirANuevoAgendamiento() {
    ...
    // Viajamos a /agendar llevando el identificador del paciente en el Query String
